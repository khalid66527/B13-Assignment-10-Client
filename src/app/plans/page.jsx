...
const SubscriptionOverviewPage = () => {
    ...
    const tiers = [
        { name: 'Free (Default)', limit: '3 paintings', price: '$0', isCurrent: true },
        { name: 'Pro', limit: '9 paintings', price: '$9.99', isCurrent: false },
        { name: 'Premium', limit: 'Unlimited', price: '$19.99', isCurrent: false }
    ];
        ...
        <div className="min-h-screen bg-[#0A0A0A] text-gray-300 py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-5xl mx-auto space-y-12">
                ...
                <div className="flex items-center gap-3 border-b border-white/5 pb-5">
                    ...
                    <div>
                        <h1 className="text-xl md:text-2xl font-extrabold text-white tracking-tight">Subscription Tier Overview</h1>
                        <p className="text-xs text-gray-500 mt-0.5">Review and select your preferred artwork purchasing tier.</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {tiers.map((tier, index) => (
                        <div 
                            key={index}
                            className={`relative overflow-hidden rounded-[2rem] p-6 border transition-all flex flex-col justify-between ${
                                tier.isCurrent 
                        ...
                        >
                            ...
                            <div className="space-y-4">
                                <div>
                                    <h3 className="text-base font-bold text-white tracking-wide">{tier.name}</h3>
                                    <div className="mt-2 flex items-baseline text-white">
                                        <span className="text-3xl font-black tracking-tight text-[#D4AF37]">{tier.price}</span>
                                        {tier.price !== '$0' && <span className="ml-1 text-xs font-semibold text-gray-500">/month</span>}
                                    </div>
                                </div>
                                ...
                                <ul className="space-y-2.5 text-xs text-gray-400">
                                    <li className="flex items-center gap-2">
                                        ...
                                        <span>Max Purchases Allowed: <strong className="text-white">{tier.limit}</strong></span>
                                    </li>
                                    <li className="flex items-center gap-2">
                                        <Icon icon="solar:check-circle-bold" className="text-[#D4AF37] size-4 shrink-0" />
                            ...
                            <button 
                                className={`w-full mt-8 py-3 rounded-xl font-extrabold tracking-wide text-xs transition-all ${
                                    tier.isCurrent 
                                    ? 'bg-gradient-to-r from-[#AA7C11] via-[#D4AF37] to-[#AA7C11] text-black shadow-md' 
                                    : 'bg-white/5 text-white hover:bg-white/10 border border-white/5'
                                }`}
                            >
                                {tier.isCurrent ? 'Active Plan' : `Upgrade to ${tier.name.split(' ')[0]}`}
                            </button>
                        </div>
                    ))}
                </div>
