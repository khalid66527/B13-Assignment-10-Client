...
export default function EditArtForm({ company, art, id }) {
  const router = useRouter();
  const [companyObj = {}] = company || [];

  // State for image link preview, initialized with artwork's existing image
  const [imagePreview, setImagePreview] = useState(art?.image || "");
  ...
  const onSubmitData = async (e) => {
    ...
    try {
      const res = await updateArt(id, updatedData);
      if (res.modifiedCount || res.acknowledged) {
        alert("Art Updated Successfully!");
        router.push('/dashboard/artist/allarts');
      } else {
        alert("Failed to update art or no changes made.");
      }
    } catch (error) {
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-fadeIn p-4 md:p-8">
      ...
      <div>
        ...
        <p className="text-xs text-gray-400 mt-1">
          Modify the details of your artwork below.
        </p>
      </div>
      ...
      <div className="bg-gradient-to-b from-[#161616]/90 to-[#0F0F0F]/95 border border-[#D4AF37]/15 rounded-3xl p-6 md:p-10 shadow-2xl relative overflow-hidden">
        ...
        <form onSubmit={onSubmitData} className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                type="date"
                required
                defaultValue={art?.date ? art.date.split("T")[0] : ""}
